...
test('Association - отображение в отладчике', () => {
  const assoc = new Association();
  ...
  const inspected = util.inspect(assoc);

  assert.ok(inspected.includes('Association'));
  assert.ok(inspected.includes('name'));
  assert.ok(inspected.includes('Test'));
  assert.ok(inspected.includes('value'));
  assert.ok(inspected.includes('42'));
});
