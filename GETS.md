...
|------|--------------------------|--------------------|
| forEach: Array.forEach (нативный) | 5.93 µs/итер | 168 690 опер/сек |
| forEach: deep(Array).forEach | 13.25 µs/итер | 75 445 опер/сек |
| forEach: Object.values + forEach (нативный) | 4.32 µs/итер | 231 539 опер/сек |
| forEach: deep(Object).forEach | 8.45 µs/итер | 118 410 опер/сек |
| forEach: String[Symbol.iterator] (нативный) | 4.18 µs/итер | 239 152 опер/сек |
| forEach: deep(String).forEach | 12.57 µs/итер | 79 580 опер/сек |
| forEach: Map.forEach (нативный) | 15.76 µs/итер | 63 436 опер/сек |
| forEach: deep(Map).forEach | 6.81 µs/итер | 146 827 опер/сек |
| forEach: Set.forEach (нативный) | 2.27 µs/итер | 441 140 опер/сек |
| forEach: deep(Set).forEach | 3.37 µs/итер | 297 170 опер/сек |
| map: Array.map (нативный) | 6.81 µs/итер | 146 884 опер/сек |
| map: deep(Array).map | 885.74 µs/итер | 1 129 опер/сек |
| map: Object.values + map (нативный) | 2.40 µs/итер | 416 617 опер/сек |
| map: deep(Object).map | 1390.82 µs/итер | 719 опер/сек |
| map: String.split + map + join (нативный) | 17.57 µs/итер | 56 908 опер/сек |
| map: deep(String).map | 869.57 µs/итер | 1 150 опер/сек |
| filter: Array.filter (нативный) | 11.81 µs/итер | 84 653 опер/сек |
| filter: deep(Array).filter | 765.11 µs/итер | 1 307 опер/сек |
| filter: Object.values + filter (нативный) | 3.11 µs/итер | 321 697 опер/сек |
| filter: deep(Object).filter | 369.55 µs/итер | 2 706 опер/сек |
