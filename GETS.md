...
|------|--------------------------|--------------------|
| Array.forEach (нативный) | 3.93 µs/итер | 254 219 опер/сек |
| deep(Array).forEach | 7.34 µs/итер | 136 297 опер/сек |
| Object.values + forEach (нативный) | 3.42 µs/итер | 292 702 опер/сек |
| deep(Object).forEach | 5.65 µs/итер | 176 966 опер/сек |
| String[Symbol.iterator] (нативный) | 2.48 µs/итер | 403 511 опер/сек |
| deep(String).forEach | 3.06 µs/итер | 326 715 опер/сек |
| Map.forEach (нативный) | 1.82 µs/итер | 548 168 опер/сек |
| deep(Map).forEach | 2.38 µs/итер | 419 592 опер/сек |
| Set.forEach (нативный) | 1.58 µs/итер | 634 692 опер/сек |
| deep(Set).forEach | 2.2 µs/итер | 453 858 опер/сек |

### map

| Тест | Среднее время выполнения | Операций в секунду |
|------|--------------------------|--------------------|
| Array.map (нативный) | 4.26 µs/итер | 234 729 опер/сек |
| deep(Array).map | 323.83 µs/итер | 3 088 опер/сек |
| Object.values + map (нативный) | 1.49 µs/итер | 671 422 опер/сек |
| deep(Object).map | 362.98 µs/итер | 2 755 опер/сек |
| String.split + map + join (нативный) | 11.19 µs/итер | 89 360 опер/сек |
| deep(String).map | 325.31 µs/итер | 3 074 опер/сек |

### filter

| Тест | Среднее время выполнения | Операций в секунду |
|------|--------------------------|--------------------|
| Array.filter (нативный) | 5.56 µs/итер | 179 894 опер/сек |
| deep(Array).filter | 358.17 µs/итер | 2 792 опер/сек |
| Object.values + filter (нативный) | 1.72 µs/итер | 579 902 опер/сек |
| deep(Object).filter | 392.77 µs/итер | 2 546 опер/сек |
